...
def get_gpu_memory_info():
    ...
    for i in range(device_count):
        ...
        print(f"GPU {i} ({name}): Total: {total:.2f} GB | Used: {used:.2f} GB | Free: {free:.2f} GB")
...
class GPT:
    def __init__(self, model_name, api=None, temperature=0, seed=0):
        ...
        try:
            ...
            import traceback
            print("🔥 Error while initializing OpenAI client:")
            traceback.print_exc()
            raise
...
# Detection Model: to check if the model's response explicitly rejects the user's request/queries (queries used from seed_reject.json file)
...
if lets_gen_finetune_dataset == True:
    ...
    for user_prompt in tqdm(attack_prompts):
        ...
        for trial in range(num_trials):
            ...
            for trial_idx in range(max_trials):
                logging.info(f"trial_idx: {trial_idx}")
    ...
    try:
        ...
        logging.error(f"Error saving ft dataset: {e}")


    logging.info(f"saveed finetune dataset to: {save_path}")
