# Test Pangea-7B with MultiJail Bengali prompts
# =============================================
...
# Cell 3: Load MultiJail Bengali Prompts
# =====================================
print("\n" + "="*60)
print("📖 LOADING MULTIJAIL BENGALI PROMPTS")
print("="*60)

# Load prompts from JSON file
with open('converted_multijail_bn.json', 'r', encoding='utf-8') as f:
    data = json.load(f)

prompts_data = data['prompts']
print(f"📊 Loaded {len(prompts_data)} prompts from converted_multijail_bn.json")
...
# Cell 4: Run MultiJail Bengali Tests
# ==================================
print("\n" + "="*60)
print("🎯 TESTING PANGEA-7B WITH MULTIJAIL BENGALI PROMPTS")
print("="*60)

# Run tests with all prompts
print("🚀 Starting batch testing...")
multijail_results = tester.batch_test(prompts, "Pangea_MultiJail_Bengali", max_new_tokens=512,temperature=0.95)

# Analyze results
print("\n📊 Pangea-7B MultiJail Bengali Analysis:")
quality_scores = tester.evaluate_quality(multijail_results, 
...
# Show sample outputs
print("\n🎨 Sample Outputs:")
for i, result in enumerate(multijail_results[:3]):
...
# Save basic results
tester.save_results_to_json("pangea_multijail_bn_results.json")

# Save evaluation results with scores
print("\n" + "="*60)
print("📊 SAVING EVALUATION RESULTS WITH SCORES")
print("="*60)
tester.save_evaluation_results_to_json("pangea_multijail_bn_evaluation_results.json")
...
total_tests = len(multijail_results)
successful_tests = len([r for r in multijail_results if not r.get('error')])
failed_tests = total_tests - successful_tests

avg_generation_time = np.mean([r['generation_time'] for r in multijail_results if not r.get('error')])
avg_tokens_per_sec = np.mean([r['tokens_per_second'] for r in multijail_results if not r.get('error')])
