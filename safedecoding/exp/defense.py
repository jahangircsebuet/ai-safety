...
# ***************LLM Models ****************
# These are the target language models whose outputs are being moderated and evaluated. 
# They represent different open-source LLM architectures on which the jailbreak attacks 
# are tested and the SafeDecoding method is applied.
...
# *************Defenders/Defense Methods***********
# Role:
# These are defense mechanisms applied to the models to prevent them from producing harmful content in response to adversarial prompts. 
# SafeDecoding is the proposed method, while the others are baseline comparisons.

# List of Defenders:
# ✅ SafeDecoding (Proposed safety-aware decoding method)
# PPL (Perplexity-based input filter)
# Self-Exam (Self-evaluation and critique)
# Paraphrase (Input paraphrasing to neutralize attack)
# Retokenization (Alter tokenization to avoid attack trigger)
# Self-Reminder (Inject safety prompts into context)
# ICD (Improved Chain-of-Denial prompting)
...
# ************Attackers/Attack Methods***********
# Role:
# These are jailbreak attack methods—crafted or optimized prompts meant to cause the LLMs to violate safety policies and generate harmful, 
# unethical, or prohibited content.

# List of Attacks/Attackers:
# GCG (Gradient-based Suffix attack)
# AutoDAN (Automated DAN-style prompt attack)
# PAIR (Prompt Auto Inversion and Replacement)
# DeepInception (Deep prompt-based attack generation)
# AdvBench (Adversarial prompts benchmark dataset)
# Customized Dataset (Hand-curated or model-specific attack prompts)

# This block loads and filters attack prompts based on the chosen attack method and target model. These prompts are used to test or evaluate 
# the robustness and safety of language models against adversarial attacks.
...
if args.attacker == "AdvBench":
    ...
    attack_prompts = load_dataset("UWNSL/SafeDecoding-Attackers", split="train")
